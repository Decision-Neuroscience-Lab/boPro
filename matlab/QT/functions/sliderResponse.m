...
lineColour = [0 113 189];
lineResponseColour = [246.993,185.997,106.0035];
numResponseColour = [246.993,185.997,106.0035];
...
if choiceTime == 0 % If indefinite response time
    ...
    while isempty(value)
        ...
        DrawFormattedText(window, text, 'center', height/5,  [0 0 0], 70, 0, 0, 2);
        ...
        Screen('DrawLine', window, lineColour, line_x, line_top, line_x, line_bottom, 3 );
        Screen(window, 'Flip');
        ...
        if keyIsDown == 1 && keyCode(downkey) == 1
            ...
            if isempty(varargin)
                for n = 1:length(markers_x)
                    number = sprintf('%0.f', n);
                    if pos == n
                        Screen('DrawText', window, number, markers_x(n) - 10, (height/2) + 60, numResponseColour);
                    else
                        Screen('DrawText', window, number, markers_x(n) - 10, (height/2) + 60, [0, 0, 0]);
                    end
                    clear number
            ...
            line_top = (height/2) + 15;
            line_bottom = (height/2) - 15;
            Screen('DrawLine', window, lineResponseColour, line_x, line_top, line_x, line_bottom, 3 );
            Screen(window, 'Flip');
