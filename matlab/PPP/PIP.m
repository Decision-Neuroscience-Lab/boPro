...
if session == 1 % If we are running the task for the first time
    ...
    sendTrigger(params,13); % Trigger baseline1 on
    params.practiceStim = params.practiceStim1;
    [data.firstPractice] = PIP_practice(params);
    sendTrigger(params,14); % Trigger baseline1 off
    ...
    sendTrigger(params,11); % Trigger practice on
    [data.rewardPractice] = reward_practice(params);
    sendTrigger(params,12); % Trigger practice off
    
    % Run the main task
    sendTrigger(params,15); % Trigger main task on
    [data.trialLog] = PIP_task(params,[]); % The second argument here is the trialLog structure, which can be blank as it is new   
    sendTrigger(params,16); % Trigger main task off
    ...
    sendTrigger(params,17); % Trigger baseline2 task on
    params.practiceStim = params.practiceStim2;
    [data.secondPractice] = PIP_practice(params);
    sendTrigger(params,18); % Trigger baseline2 task off
    ...
    lastTrial = sum(~isnan([data.trialLog.bisectRt]),2);
    params.startTrial = lastTrial;
     
    % Run the main task
    sendTrigger(params,15); % Trigger main task on
    [data.trialLog] = PIP_task(params,data.trialLog); % The second argument here is the trialLog structure, which can be blank as it is new   
    sendTrigger(params,16); % Trigger main task off
...
% Launch Qualtrics
url = sprintf('https://fbeunimelb.asia.qualtrics.com/SE/?SID=SV_9vk8C0ltugBiTAh&ParticipantID=%.0f',id); % Append ID to URL (if ID is empty for whatever reason, survey will prompt for ID entry)
web(url,'-browser')
return