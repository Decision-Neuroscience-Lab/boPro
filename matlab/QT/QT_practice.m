function [trialLog] = QT_practice(params)
try
    ...
    while GetSecs <= blockStartTime + params.blockTime
        ...
        while GetSecs <= blockStartTime + params.blockTime
            ...
            drawPracStim(params, totalReward, GetSecs - blockStartTime, matured, 0);
            ...
            if keyIsDown && isnan(rt) % Key press
                ...
                while GetSecs < trialStartTime + rt + (params.iti/2) && GetSecs <= blockStartTime + params.blockTime
                    drawPracStim(params, totalReward, GetSecs - blockStartTime, matured, 1);
                    Screen('Flip', params.window);
                end
                break; % Break out of trial
            end
        ...
        trialLog(t,:) = {t, cellstr(params.d.DistributionName), delay, rt, logical(censor), reward, totalReward, (GetSecs - blockStartTime)};
        ...
        while GetSecs < itiStart + (params.iti/2) && GetSecs <= blockStartTime + params.blockTime
            ...
            total = sprintf('(%.0f points total)', totalReward*100);
