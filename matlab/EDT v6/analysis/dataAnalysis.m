...
for x = participants
    ...
    temp = dataMatrix(dataMatrix(:,1) == 1 & dataMatrix(:,2) == c,:);
    [E(c,1:2), E(c,3), ~] = fitTD(temp(:,4), temp(:,5), temp(:,6), temp(:,7), temp(:,8), 'exponential');
    [H(c,1:2), H(c,3), ~] = fitTD(temp(:,4), temp(:,5), temp(:,6), temp(:,7), temp(:,8), 'hyperbolic');
    c = c + 1;
end
