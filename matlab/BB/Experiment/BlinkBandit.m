function [trialLog params] = BlinkBandit
...
params.itiMean = 1; %mean of poisson distribution from which to draw intertrial interval
...
trialLog.participant = input('Enter participant number: ');
...
% Send experiment start trigger (1)
RTBox('TTL',1);
...
savename = sprintf('%sBLINKBANDIT_DATA_p%.0f', params.saveDir, trialLog(1).participant);
save(savename, 'trialLog', 'params');

% Tell participants it's the end 
tts('That''s all folks. Thank you', params.speed);
WaitSecs(1)
